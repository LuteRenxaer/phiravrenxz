...
预编译版本发布在 GitHub Releases。当前最新为 1.3.2（2026-09-16）。
...
> 上面卡片里的 1.3.2（2026-09-16）就是当前的发行版。多人房间页细节、首启向导、Flick 判定、全屏适配这些改动**尚未发行**，下载发行版是玩不到的，想用得看[构建指南](phira_build_guide/index.md)自行构建。改动清单一并放在[与 Phira 的区别](changes/index.md)里。

## 多人联机 {#multiplayer}

游戏内置 Phira-MP 客户端，也支持自建服务器。

> 这一节写的是源码里的多人流程（没有观战、没有结算页、大厅与连接页改版等），**尚未随发行版发布**，需要自行构建才能用到。
...
源码里新增了首启向导：首次启动走一遍 语言 → 登录 → 音量 → 其他设置 → 最后确认，最后一步会问要不要玩新手教程，老玩家（数据里已经选过语言的）不会重走。这个向导**尚未随发行版发布**，需要自行构建；发行版里照旧在设置页里改。
